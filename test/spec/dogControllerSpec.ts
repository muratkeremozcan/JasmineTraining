...
<<<<<<< HEAD
        ...
        beforeEach(() => {
          ...
          throwObject.name = 'meh';  // properties with default values
          throwObject.flies = false;
          throwObject.chewy = false;
          (<jasmine.Spy>(throwObject.chewOn)).and.returnValue(  // return value for chewOn spy
            vdog.ChewExperience.fair);
          sut.blogContent = '';  // want blog content empty prior to throw event
        });
        it('should blog master', () => {
          $rootScope.$broadcast(eventNamesTest.masterThrow, throwObject);  // broadcast the masterThrow event, passing in throwObject
          expect(sut.blogContent).toContain('master');  // we expect our blogContent to containt the word master
        });
